...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        printf("usage: %s <file>   --- transfer file to usb device\n", argv[0]);
        return 0;
    }
    else
    {
    ...
    while (len > 0)
    {
        r = libusb_bulk_transfer(handle, ENDPOINT_ADDRESS | LIBUSB_ENDPOINT_OUT, data, len, &transferred, 5000);
    ...
    r = libusb_bulk_transfer(handle, ENDPOINT_ADDRESS | LIBUSB_ENDPOINT_IN, data, sizeof(data), &transferred, 5000); // 替换ENDPOINT_ADDRESS为你的endpoint地址，例如0x82表示地址2的IN endpoint
    if (r == 0 && transferred > 0) {
        printf("收到 %d 字节的数据\n", transferred);
        dump_data(data, transferred);
    } else {
