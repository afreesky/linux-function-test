...
#define wdm_trace_write(desc, count, rv) \
	trace_printk("wdm_write: minor=%d count=%d rv=%d\n", desc->intf->minor, (int)(count), rv)

#define wdm_trace_read(desc, count, rv) \
	trace_printk("wdm_read: minor=%d count=%d rv=%d\n", desc->intf->minor, (int)(count), rv)

#define wdm_trace_probe(intf, rv) \
	trace_printk("wdm_probe: minor=%d rv=%d\n", intf->minor, rv)

#define wdm_trace_disconnect(intf) \
	trace_printk("wdm_disconnect: minor=%d\n", intf->minor)

#define wdm_trace_urb_submit(desc, urb_type, rv) \
	trace_printk("wdm_urb_submit: minor=%d urb=%s rv=%d\n", desc->intf->minor, urb_type, rv)

#define wdm_trace_urb_complete(desc, urb_type, status, length) \
	trace_printk("wdm_urb_complete: minor=%d urb=%s status=%d length=%d\n", desc->intf->minor, urb_type, status, length)

#define wdm_trace_in_data(desc, length) do { \
	if (length > 0) { \
		char __buf[64]; \
		int __i, __len = min(length, 16); \
		__buf[0] = '\0'; \
		for (__i = 0; __i < __len; __i++) \
			sprintf(__buf + __i * 3, "%02x ", desc->inbuf[__i]); \
		trace_printk("wdm_in: %d bytes: %s\n", length, __buf); \
	} \
} while (0)
...
static void wdm_in_callback(struct urb *urb)
{
	...
	wdm_trace_urb_complete(desc, "read", status, length);
	...
	if (length > 0) {
		char buf[64];
		int i, len = min(length, 16);
		buf[0] = '\0';
		for (i = 0; i < len; i++)
			sprintf(buf + i * 3, "%02x ", desc->inbuf[i]);
		buf[min(len * 3 - 1, (int)(sizeof(buf) - 2))] = '\0';
		wdm_trace_in_data(desc, length);
	}
...
static void wdm_int_callback(struct urb *urb)
{
	...
	if (!desc->resp_count++ && !responding
		...
		wdm_trace_urb_submit(desc, "response", rv);
...
(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	wdm_trace_urb_submit(desc, "write", rv);
	if (rv < 0) {
		desc->outbuf = NULL;
		clear_bit(WDM_IN_USE, &desc->flags);
